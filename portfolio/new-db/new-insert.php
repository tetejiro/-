...
*/
    ...
    function insertDb3($whose, $task, $bytime1, $bytime2, $emotion, $time1, $time2, $attention, $strong1, $strong2, $strong3)
    {
        $ConstDb = new ConstDb();
        $dsn = ConstDb::dsn;
        $user = ConstDb::user;
        ...
        try
        {
            ...
            $sql='INSERT INTO now (whose,task,bytime1,bytime2,emotion,time1,time2,attention,strong1,strong2,strong3)
                    VALUES (trim(:whose), trim(:task), trim(:bytime1), trim(:bytime2), trim(:emotion), trim(:time1), trim(:time2), trim(:attention), trim(:strong1), trim(:strong2), trim(:strong3))';
            $stmt = $dbh->prepare($sql);
            $stmt->bindValue(':whose', $whose, PDO::PARAM_INT);
            $stmt->bindValue(':task', $task, PDO::PARAM_STR);
            $stmt->bindValue(':bytime1', $bytime1, PDO::PARAM_INT);
            $stmt->bindValue(':bytime2', $bytime2, PDO::PARAM_INT);
            $stmt->bindValue(':emotion', $emotion, PDO::PARAM_STR);
            $stmt->bindValue(':time1', $time1, PDO::PARAM_INT);
            $stmt->bindValue(':time2', $time2, PDO::PARAM_INT);
        ...
        //hozon.phpのインサート
    function insertDb4($honnin, $code, $situation, $goal, $what, $why, $try)
    {
        $ConstDb = new ConstDb();
        $dsn = ConstDb::dsn;
        $user = ConstDb::user;
        ...
        try
        {
            ...
            $sql='INSERT INTO question (whose, whom, situation, goal, what, why, try0)
                    VALUES (:honnin, :code, :situation, :goal, :what, :why, :try)';
            $stmt = $dbh->prepare($sql);
